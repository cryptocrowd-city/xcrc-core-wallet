...
**As a fundamental rule, game engines should only ever depend on CRyptoCrowd
...
An alternative to handling XCRC payments is to require **burning of XCRC**.
...
of payments).  To burn XCRC in relation to a given move in a game,
...
a single burn of XCRC could be used for multiple games at the same time.)
