...
placing **restrictions on receiving XCRC on certain addresses**.  This
facilitates building in-game market places.

## Overview

CRyptoCrowd games [can react to voluntary transfers of XCRC](games.md#currency), but
they cannot force transfers.  This still allows an individual game to
include a market place for trading assets with XCRC within its rules:
...
is unique, it could happen that two prospective buyers send XCRC at roughly
...
transaction that does not yet transfer any XCRC but signals her intent to
buy the item.  After confirmation, the item is reserved for her for a certain
time (e.g. 100 blocks).  During that time, Alice can then transfer the XCRC
...
XCRC that would have to be refunded.
...
specify that they want to receive only a single XCRC transaction at their
...
* Each restriction consists of a script (address) on which it is placed,
  ...
  transactions are allowed or how much XCRC can be sent in total).
...
* Address restrictions *do not apply to name transactions* sent to
  the address.  In other words, the locked 0.01 XCRC in a name's
  "coloured coin" are not counted towards the limits.
* Name updates with JSON for an invalid restriction or transfers of XCRC
...
* The actual data about the restriction.  This can be one or more of the
  ...
  * Maximum amount `A` of XCRC to be received
  * Minimum amount `s` of XCRC sent in any single transaction
